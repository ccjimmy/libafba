...
{
#ifdef ANDROID
	printf( "SDLSoundGetNextSoundFiller" );
#endif
...
{
#ifdef ANDROID
	int nInitedAudio = SDL_WasInit(SDL_INIT_AUDIO);

	if (!(nInitedAudio & SDL_INIT_AUDIO)) 
	{
	...
	printf( "nSoundFps: %i, nAudSegLen: %i, nAudLoopLen: %i, nSDLBufferSize: %i", nSoundFps, nAudSegLen, nAudLoopLen, nSDLBufferSize );
